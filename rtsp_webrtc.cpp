...
on_offer_created (GstPromise * promise, gpointer user_data)
{
    ...
    const gchar *text_fmtp = gst_sdp_media_get_attribute_val ((GstSDPMedia *)&g_array_index(offer->sdp->medias, GstSDPMedia, 0), "fmtp");
    if(strstr(text_fmtp, "profile-level-id") != NULL){
        ...
        guint attr_len = gst_sdp_media_attributes_len ((GstSDPMedia *)&g_array_index(offer->sdp->medias, GstSDPMedia, 0));
        printf("Attributes Length: %d\n", attr_len);
        guint fmtp_index;
        for(guint index=0; index<attr_len; index++){
            const GstSDPAttribute *gstSDPAttribute = gst_sdp_media_get_attribute ((GstSDPMedia *)&g_array_index(offer->sdp->medias, GstSDPMedia, 0), index);
        ...
        if(fmtp_index > 0){
            ...
            gst_sdp_media_remove_attribute ((GstSDPMedia *)&g_array_index(offer->sdp->medias, GstSDPMedia, 0), fmtp_index);
            gst_sdp_media_add_attribute((GstSDPMedia *)&g_array_index(offer->sdp->medias, GstSDPMedia, 0), "fmtp", "96 packetization-mode=1;profile-level-id=42e01f;level-asymmetry-allowed=1;sprop-parameter-sets=Z0IAH5Y1QKALdgLcBAQFAAADA+kAAOpghA==,aM48gA==");
...
start_pipeline (void)
{
    ...
    GArray *transceivers;
    //pipe1 = gst_parse_launch ("rtspsrc location=rtsp://10.142.138.151/z3-1.mp4 latency=10 drop-on-latency=TRUE ! rtph264depay ! avdec_h264 ! x264enc ! rtph264pay ! application/x-rtp,media=video,encoding-name=H264,payload=96 ! webrtcbin name=sendrecv", &error);
    pipe1 = gst_parse_launch ("rtspsrc location=rtsp://127.0.0.1:8554/test latency=10 drop-on-latency=TRUE ! rtph264depay ! rtph264pay config-interval=1 ! application/x-rtp,media=video,encoding-name=H264,payload=96 ! webrtcbin name=sendrecv", &error);
    if (error) {
